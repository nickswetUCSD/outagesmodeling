...
We thought that an interesting metric to test for fairness would be across whether or not the climate for a blackout was normal. We noticed that if you added up all of the rows where CLIMATE.CATEGORY 🌤 is either 'Cold' or 'Warm, you get a similarly sized dataframe to where CLIMATE.CATEGORY 🌤 is 'Normal'.
